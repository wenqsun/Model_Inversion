...
def inversion(args, classifier, evaluator, Generator, Disc, labels, transform_resize, device):
    ...
    plt.figure()
    plt.plot(np.arange(500, (args.num_epoch+1), 500), fid_list)
    plt.xlabel('epoch')
    ...
    plt.figure()
    plt.plot(np.arange(500, (args.num_epoch+1), 500), attack_acc_list)
    plt.xlabel('epoch')
    plt.ylabel('attack_acc')
    plt.savefig(attack_acc_path)
    
    # save the fid score and attack accuracy
...
if __name__ == '__main__':
    ...
    inversion(args, classifier, evauator, Generator, Disc, labels, transform_resize, device)
