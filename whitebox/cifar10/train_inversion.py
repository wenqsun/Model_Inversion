...
def inversion(args, classifier, evaluator, Generator, Disc, labels, transform_resize, target_acc, device):
    ...
    for epoch in range(1, (args.num_epoch+1)):
        ...
        if epoch % args.log_interval== 0:
            ...
            z = Variable(z, requires_grad=True).cuda()
            labels = torch.LongTensor(args.data_num).random_(0, 10).cuda()
            gt_labels = torch.zeros(args.data_num, 10).cuda()
...
if __name__ == '__main__':
    ...
    classifier = FedAvgCNN().cuda()      # target model
    evauator = FedAvgCNN().cuda()      # evaluator: evaluate the quality of the generated images, a model with high accuracy
    Generator = InverseCIFAR10Net(z_dim = args.z_dim, num_class=10).cuda()         # generator
    ...
    evauator = torch.load('result_model/Central/dir_0.1_model.pth')       # load the evaluator
    labels = torch.LongTensor(args.batch_size).random_(0, 10).cuda()   # generate random labels
